                  ...
                  <Col span={16}>
                    ...
                    >
                      <Select
                        ...
                        style={{ width: "100%" }}
                      >
                        <Select.OptGroup label="Consignment Types">
                          {Object.entries(ConsignSaleType)
                            .filter(
                              ([key]) =>
                                key === "ConsignedForSale" ||
                                key === "ConsignedForAuction"
                            ) // Include only the desired types
                            .map(([key, type]) => (
                              <Select.Option key={key} value={type}>
                                {type.replace(/([A-Z])/g, " $1").trim()}{" "}
                                {/* Convert camel case to spaced format */}
                              </Select.Option>
                            ))}
                        </Select.OptGroup>
                      </Select>
